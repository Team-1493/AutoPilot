...
class Autopilot:
    ...
    def atTarget(self,  current: Pose2d,  target: ap_target):
        ...
        okXY = hypot(current.X() - goal.X(),current.Y() - goal.Y()) <= self.m_profile.errorXY
        okTheta = abs(current.rotation().radians() - (goal.rotation()).radians()) <= self.m_profile.errorTheta
